...
状态：公开 Release Candidate；官方文档校订与自动回归已完成，真实账户试走持续补充
...
| 支付方式 | 官方当前列出银行卡、PayPal 和 Alipay 等方式；不保证每个中国账户均可使用 | [Vultr payment method](https://docs.vultr.com/support/platform/billing/why-do-i-need-to-enter-a-payment-method) | 编写支付章前 |
...
| Vultr 两因素认证 | 当前支持 TOTP 与 FIDO2/WebAuthn；官方建议预先生成 Emergency Backup Codes | [Vultr 2FA](https://docs.vultr.com/support/platform/authentication/what-is-two-factor-authentication) | 每次发布前 |
...
| 预授权、美元账单与税务地区 | 银行卡可能出现临时验证授权；账单以 USD 出具；税务地区参考账单地址与主要支付方式 | [Vultr billing FAQ](https://docs.vultr.com/platform/billing/faq)、[Tax location](https://docs.vultr.com/support/platform/billing/how-is-my-tax-location-determined) | 编写第 9 章及每次发布前 |
...
| 创建页面核心字段 | 当前流程包含 Shared CPU、Location、Plan、Operating System、SSH Keys、Firewall Group、Public IPv4、Automatic Backups 和 Limited User Login | [Vultr provisioning](https://docs.vultr.com/products/compute/instances/cloud-compute/provisioning) | 使用测试账户逐字段复核 |
...
| Vultr Firewall | Firewall Rule 可按 IPv4、TCP、来源地址和端口过滤流量；第一册只配置 TCP 22 | [Vultr firewall rules](https://docs.vultr.com/products/network/firewall-groups/management/rules) | 使用测试实例验证 My IP 与动态 IP 分支 |
...
## 不阻塞公开 Release Candidate 的实机复核项

以下项目用于核对厂商当前界面、账户差异和真实网络结果。正文已经使用教学仿真、
条件化表达、校订日期和官方来源控制风险，因此这些项目不阻止当前版本公开阅读；
完成后应把脱敏结论回填到本台账，并只在事实确有变化时修改正文。

其中“中国大陆网络访问”只表示尚未覆盖对应运营商和网络环境，不表示已经观察到
地域封锁或普通浏览器 403。只有真实测试或稳定的读者反馈显示注册、控制台、支付或
实例管理无法使用时，才把它升级为发布阻塞问题。

- [ ] 使用中国大陆网络访问 Vultr 注册与控制台；
- [ ] 使用测试账户核对当前 Alipay、银行卡和 PayPal 分支；
- [ ] 使用测试账户完成唯一密码、邮箱验证、2FA 与恢复码流程；
- [ ] 核对额外身份验证或人工审核的实际提示；
- [ ] 核对东京地区当前可售 Shared CPU 套餐；
- [ ] 使用中国大陆家庭宽带和至少一种移动网络实测东京、洛杉矶 Looking Glass；
- [ ] 核对所选套餐的 vCPU、RAM、磁盘、流量、独立公网 IPv4 和价格；
- [ ] 核对 Ubuntu 24.04 LTS x64 镜像与默认用户名；
- [ ] 分别使用密码和 ED25519 密钥部署测试实例；
- [ ] 在 Windows 与 macOS 分别生成 ED25519 密钥并核对默认文件路径；
- [ ] 核对创建页面的 SSH Key、Limited User Login、Public IPv4、Automatic Backups 和 Deploy 摘要；
- [ ] 核对云防火墙 TCP 22 与来源 IP 分支；
- [ ] 核对 Deploy 最终摘要中的小时价、月度参考价、币种、Public IPv4 和全部附加项；
- [ ] 只提交一次测试 Deploy，并确认 Billing History 只出现一个实例；
- [ ] 在测试账户核对 Automatic Backups Off、快照、Bandwidth Usage 和发票入口；
