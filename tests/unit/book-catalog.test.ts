...
describe("系列书籍注册表", () => {
  it("保留十册规划并公开前五册发布候选", () => {
    ...
    const thirdBook = getBookById("03-docker");
    expect(thirdBook).toBeDefined();
    expect(thirdBook?.status).toBe("release-candidate");
    ...
    const fifthBook = getBookById("05-open-webui");
    expect(fifthBook).toBeDefined();
    expect(fifthBook?.status).toBe("release-candidate");
    expect(fifthBook?.version).toBe("1.0.0-rc.1");
    expect(fifthBook?.search.enabled).toBe(true);
    expect(fifthBook?.print.enabled).toBe(true);

    const sixthBook = getBookById("06-dify");
    expect(sixthBook).toBeDefined();
    expect(sixthBook?.status).toBe("release-candidate");
