...
const bookPath = "books/05-open-webui/";

test.describe("第五册发布候选验收", () => {
  test("封面、完整目录、Open WebUI 主线、章节导航和打印入口", async ({
    page,
  }) => {
    ...
    await expect(page.locator("main h1")).toContainText(/搭建自己的 AI 对话入口/);
    ...
    await page.getByRole("link", { name: "通过 Tunnel 公开" }).click();
    await expect(page).toHaveURL(/\/07-publish-with-tunnel\/$/);
    await expect(page.getByRole("heading", { level: 1 })).toContainText(
      "用独立子域名发布 AI 对话入口",
    );
    await expect(page.locator(".tunnel-route-mock")).toBeVisible();
    await expect(page.locator(".code-block")).toHaveCount(5);

    await page.getByRole("link", { name: /下一章/ }).click();
    await expect(page).toHaveURL(/\/08-first-conversation\/$/);
    await expect(page.getByRole("heading", { level: 1 })).toContainText(
      "完成第一次真实对话",
    );
  ...
  test("关闭 JavaScript 后排错正文、命令与打印内容仍可阅读", async ({
    ...
    await expect(page.locator(".troubleshooting-item")).toHaveCount(8);
    expect(
  ...
  for (const viewport of [
    ...
    test(`${viewport.width} × ${viewport.height} 的发布章无页面级横向溢出`, async ({
      page,
      browserName,
    }) => {
      test.skip(browserName !== "chromium");
      await page.setViewportSize(viewport);
      await page.goto(`${bookPath}07-publish-with-tunnel/`);
  ...
  test("代表页面无 WCAG A/AA 自动化违规", async ({
    ...
    for (const path of [
      bookPath,
      `${bookPath}04-deploy-open-webui/`,
      `${bookPath}07-publish-with-tunnel/`,
